        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $sql2 = "SELECT class_id FROM `student` WHERE sid='$class_joined'";
                    $result2 = mysqli_query($conn, $sql2);
                    echo '<div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800" style="padding-left: 10px;">My ' . $class_joined . ' All Pending Assignments</h1>
                    </div>
                ...
                <div class="container px-5">
                    ...
                    $sql = "SELECT * FROM `schedule_assignment`, student WHERE `assignment_id` not in (SELECT `assignment_id` from solved_assignment) and `assignment_class` = '$class_joined' and student.sid = $s_id";
                    $result = mysqli_query($conn, $sql);
                    $noResult = true;
                    while ($row = mysqli_fetch_assoc($result)) {
                        $noResult = false;
                        $assignment_name = $row['assignment_name'];
                        $assignment_info = $row['assignment_info'];
                        $before_time = $row['before_time'];
                        $due_date = $row['due_date'];

                        echo '  <div class="card shadow mb-4">
                            <div class="card-body">
                                <h3 class="card-title mb-4"> ' . $assignment_name . '</h3>                              
                                <p class="card-text">' . $assignment_info . '</p>
                                
                                <div class="d-flex">
                                <h6>Before :' . $before_time . '</h6>                   
                                                                    
                            </div>
                            <div class="d-flex">
                                                 
                                <h6 class="card-subtitle mb-2 text-muted">Due Date: ' . $due_date . '</h6>                                      
                            </div>
                                <div class="row">
                                    <div class="col-md-6 py-2">
                                    <a href="solve_assignment.php"><button type="button" class="btn btn-primary">Submit Assignment</button></a>
                                    </div>                                        
                                </div>
                                <div class="d-flex">
                    ...
                    if ($noResult) {
                        echo '<div class="card shadow mb-4">
                            <div class="card-body">
                                <h3 class="card-title mb-4">No assignments Found</h3>                               
                            </div>
                        </div>';
                    }
